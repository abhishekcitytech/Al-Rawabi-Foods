//
//  mysubscriptionlineviewproductlist.swift
//  AlRawabi
//
//  Created by SandipanMacmini on 16/11/22.
//

import UIKit

class mysubscriptionlineviewproductlist: UIViewController,UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource
{
    ...
    func postAddProductApiMethod(strqty:String,strproductid:String,strprice:String)
    {
        let myAppDelegate = UIApplication.shared.delegate as! AppDelegate
        DispatchQueue.main.async {
            self.view.activityStartAnimating(activityColor: UIColor.white, backgroundColor: UIColor.clear)
        ...
        let strconnurl = String(format: "%@%@", Constants.conn.ConnUrl, Constants.methodname.apimethod55)
        let request = NSMutableURLRequest(url: NSURL(string: strconnurl)! as URL)
