//
//  maidSelectShippingAddress.swift
//  AlRawabi
//
...
class maidSelectShippingAddress: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    ...
    @IBAction func presspaycheckout(_ sender: Any)
    {
        ...
        let ctrl = maidpaymentmethod(nibName: "maidpaymentmethod", bundle: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if tableView == tabvmyaddress
        {
            ...
            cell.btndetailsarrow.isHidden = true
                ...
                if let json = try JSONSerialization.jsonObject(with: data!) as? NSDictionary
                {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if strsuccess == true
                        {
                            ...
                            let arrmaddress = dictemp.value(forKey: "customerDetails") as? NSArray ?? []
    ...
    func getshippingmethodlist()
    {
        ...
        var strconnurl = String()
        strconnurl = String(format: "%@%@?language=%@", Constants.conn.ConnUrl, Constants.methodname.apimethod34,strLangCode)
        let request = NSMutableURLRequest(url: NSURL(string: strconnurl)! as URL)
        request.httpMethod = "GET"
        request.setValue("Bearer \(strbearertoken)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
                ...
                if let json = try JSONSerialization.jsonObject(with: data!) as? NSDictionary
                {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if strsuccess == true
                        {
                            ...
                            if self.arrMShippingmethodlist.count == 0{
                                self.msg1 = "No shipping method found!"
                            }
